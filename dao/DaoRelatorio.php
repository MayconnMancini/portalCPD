...
*/
  ...
  public function reparosPorMes(string $ano): array {
    $sql = "SELECT MONTH(rep.dataInicioReparo) mes, Count(*) total 
            FROM pc_reparos rep
            WHERE YEAR(rep.dataInicioReparo) = ?
            GROUP BY MONTH(rep.dataInicioReparo) 
            ORDER BY MONTH(rep.dataInicioReparo)";
    ...
    if ($stmt) {
      ...
      if ($stmt->execute()) {
        ...
        while($stmt->fetch()) {
          ...
          // Adotei a abordagem abaixo por ser mais rapido, 
          // mas nao eh eficiente
          $reparos[] = array("mes" => $mes, "total" => $total);
        }
      }
      $stmt->close();
    }
